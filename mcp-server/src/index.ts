  ...
  inputSchema: {
    ...
    properties: {
      ...
      operation: { 
        ...
        description: "Operation to perform: identify (recognize face), add_face (add new person), list_faces (get all known faces)" 
      },
      name: { 
        type: "string", 
...
async function handleFaceRecognition(args: any) {
  ...
  try {
    switch (operation) {
      case "identify":
        const recognitionResult = await pythonFaceRecognitionService.recognizeFace(image_data);
        return {
          content: [{
            type: "text",
      ...
      case "add_face":
        ...
        const imageBuffer = await pythonFaceRecognitionService.processImage(image_data);
        const faceDetections = await pythonFaceRecognitionService.detectFaces(imageBuffer);
  ...
  try {
    ...
    try {
      ...
      if (error) {
        ...
        console.warn("Face recognition database features may not work properly");
      } else {
    ...
    console.log("   - recognize_face (face identification, adding faces, listing faces)");
